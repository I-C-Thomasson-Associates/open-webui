<script lang="ts">
	...
	import { getUserUsageLimit, type UserUsageLimitResponse } from '$lib/apis/usage';
	...
	let loading = true;
	let usageLimit: UserUsageLimitResponse | null = null;
	let usageLimitLoading = true;
	...
	$: usageLimitPercent = usageLimit?.percent ?? null;
	$: usageLimitBarWidth = usageLimitPercent !== null ? Math.min(usageLimitPercent, 100) : 0;
	$: usageLimitBarColor =
		usageLimitBarWidth >= 90
			? 'bg-[#da3e44]'
			: usageLimitBarWidth >= 50
				? 'bg-amber-400/80'
				: 'bg-emerald-500/80';
	...
	const loadUsageLimit = async () => {
		usageLimitLoading = true;
		try {
			usageLimit = await getUserUsageLimit(localStorage.token);
		} catch (error) {
...
<div class="flex h-full min-h-0 flex-col">
	...
	<div class="scrollbar-hover min-h-0 flex-1 overflow-y-auto pr-1.5">
		<UserSettingSection title={$i18n.t('Monthly AI Allowance')} first>
			...
			{:else}
				<div class="mb-2 flex items-center justify-between gap-3">
					<div class="text-xs text-gray-500 dark:text-gray-400">{$i18n.t('Monthly usage')}</div>
					{#if usageLimit.tier}
						<div class="rounded-full bg-gray-50 px-2 py-0.5 text-xs text-gray-500 dark:bg-gray-850">
							{usageLimit.tier}
						</div>
					{/if}
				</div>
				<div class="flex items-center gap-3">
					<div class="h-2 flex-1 overflow-hidden rounded-full bg-gray-100 dark:bg-gray-800">
						<div
							class="h-full rounded-full {usageLimitBarColor} transition-all"
							style="width: {usageLimitBarWidth}%"
						></div>
					</div>
					<div class="w-10 text-right text-xs font-medium">{usageLimitPercent}%</div>
				</div>
				<div class="mt-2 text-xs text-gray-500 dark:text-gray-400">
					{#if usageLimitPercent >= 100}
						{$i18n.t("You've reached your monthly usage limit.")}
					{:else if usageLimit.tier}
						{$i18n.t("You've used {{percent}}% of your monthly AI usage on {{tier}}.", {
							percent: usageLimitPercent,
							tier: usageLimit.tier
						})}
					{:else}
						{$i18n.t("You've used {{percent}}% of your monthly AI usage.", {
							percent: usageLimitPercent
						})}
					{/if}
					{$i18n.t('Resets {{date}}.', { date: formatResetDate(usageLimit.reset_at) })}
				</div>
				<div class="mt-2 text-xs text-gray-500 dark:text-gray-400">
					{$i18n.t('To be upgraded to the next usage tier, please contact your manager.')}
				</div>
		...
		{:else if !usage}
			<div class="flex min-h-32 items-center justify-center text-xs text-gray-500">
				{$i18n.t('Failed to load usage')}
			</div>
		{:else}
			<UserSettingSection title={$i18n.t('Overview')}>
