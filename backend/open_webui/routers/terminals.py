...
from open_webui.ext.terminal_context_authorization import authorized_terminal_chat_context
...
async def proxy_terminal(
    ...
    if session_id:
        ...
        authorized_chat_id = await authorized_terminal_chat_context(user, session_id)
        context_id = terminal_context_id(connection, {'chat_id': authorized_chat_id}, 'chat')
...
async def _resolve_authenticated_connection(ws: WebSocket, server_id: str):
    ...
    return user, connection, authorized_chat_id or '', token
...
async def ws_terminal(
    ...
    upstream_headers = {}
    if terminal_context_config(connection, 'chat').get('context_id') == 'chat_id' and not context_id:
        await ws.close(code=4003, reason='An accessible saved chat is required for this terminal')
        return
    if context_id:
