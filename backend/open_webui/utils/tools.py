        ...
        if tool:
            ...
            if tool_id.startswith('server:'):
                ...
                if type == 'openapi':
                    ...
                    function_name_filter_list_raw = tool_server_connection.get('config', {}).get(
                        'function_name_filter_list', ''
                    )

                    # Config backends may return either a JSON list or a comma-separated string.
                    if isinstance(function_name_filter_list_raw, list):
                        function_name_filter_list = [
                            str(item).strip() for item in function_name_filter_list_raw if str(item).strip()
                        ]
                    elif isinstance(function_name_filter_list_raw, str):
                        function_name_filter_list = [
                            item.strip() for item in function_name_filter_list_raw.split(',') if item.strip()
                        ]
                    else:
                        function_name_filter_list = []
